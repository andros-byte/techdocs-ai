...
# 3. Головний інтерфейс (Screenshot_66)
...
# Оновлений приклад про інвертор Deye за твоїм запитом
...
if user_query:
    ...
    # ПРОФЕСІЙНА СИНЯ КНОПКА (замість жовтої)
    st.markdown(f'''
    ...
    if st.button("Отримати рекомендацію (я вже оплатив)"):
        with st.spinner("Формуємо вашу персональну інструкцію..."):
            try:
                # Використовуємо твій баланс $4.88
                response = openai.ChatCompletion.create(
                    model="gpt-3.5-turbo",
                    messages=[
                        {"role": "system", "content": "Ти професійний технічний консультант. Твої відповіді мають бути чіткими та структурованими."},
                ...
                st.info("Перевірте API Key (Баланс: $4.88).")

else:
    # Повідомлення на порожньому екрані (Screenshot_66)
    st.warning("Будь ласка, спочатку опишіть вашу проблему вище, щоб відкрити розділ оплати.")
